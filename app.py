...
if uploaded_file is not None:
    ...
    # Xác định các cột giờ (có "Vào" hoặc "Ra" trong tên cột)
    time_cols = [col for col in df.columns if ("Vào" in str(col) or "Ra" in str(col))]
    ...
    # Chuẩn hóa giờ cho dữ liệu gốc trước khi xử lý tiếp (giúp các sheet xuất ra không dính giây)
    df_copy = df.copy()
    for col in time_cols:
        df_copy[col] = df_copy[col].apply(to_hhmm)
    ...
    # Chuẩn hóa giờ cho dữ liệu đã lọc
    for col in time_cols:
    ...
    st.subheader("Dữ liệu đã lọc, thêm cột Thứ (giờ chuẩn HH:mm):")
    ...
    if st.button("Tách và xuất Excel tổng (sheet 1 = gốc, sheet 2+ = từng nhân viên)"):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            df_copy.to_excel(writer, sheet_name="Du_lieu_goc", index=False)
            # Các sheet nhân viên đã xử lý
            for (ma_nv, ho_ten), group in groupby_obj:
                group = group.copy()
                # Chuẩn hóa lại giờ một lần nữa cho chắc ăn
                for col in time_cols:
                    group[col] = group[col].apply(to_hhmm)
                # Tính tổng các cột từ "Lương giờ 100%" trở đi
                total_row = {}
